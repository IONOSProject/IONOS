...
#include "ui_zionoscontroldialog.h"
...
std::list<std::string> ZIonosControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZIonosControlDialog::listMints;

ZIonosControlDialog::ZIonosControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZIonosControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZIonosControlDialog::~ZIonosControlDialog()
{
    delete ui;
}

void ZIonosControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZIonosControlDialog::updateList()
{
...
void ZIonosControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZIonosControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZIonos_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZIonosControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZIonosControlDialog::GetSelectedMints()
{
...
void ZIonosControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
