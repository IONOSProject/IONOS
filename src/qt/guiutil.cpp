...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Ionos address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no Ionos: URI
...
{
    // Convert ionos:// to ionos:
    //
    //    Cannot handle this later, because ionos:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open ionos.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Ionos.lnk";
}
...
{
    return GetAutostartDir() / "ionos.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ionos.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Ionos\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ionos app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ionos app to startup item list
