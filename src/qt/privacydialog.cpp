                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zIOX ought to be enough for anybody." - Bill Gates, 2017
    ui->zIOXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzIOXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzIOX->setEnabled(false);
        ui->pushButtonMintzIOX->setToolTip(tr("zIOX is currently disabled due to maintenance."));

        ui->pushButtonSpendzIOX->setEnabled(false);
        ui->pushButtonSpendzIOX->setToolTip(tr("zIOX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zIOXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzIOX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zIOX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zIOX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zIOX in ") +
...
void PrivacyDialog::on_pushButtonSpendzIOX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zIOX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zIOX
        sendzIOX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zIOX
    sendzIOX();
}

void PrivacyDialog::on_pushButtonZIonosControl_clicked()
{
    ZIonosControlDialog* zIonosControl = new ZIonosControlDialog(this);
    zIonosControl->setModel(walletModel);
    zIonosControl->exec();
}

void PrivacyDialog::setZIonosControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzIonosSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzIOX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Ionos Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zIOXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zIOX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " IOX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zIOX</b>";
    ...
    // use mints from zIonos selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZIonosControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZIonosControlDialog::GetSelectedMints();
    }
    ...
    // Clear zionos selector in case it was used
    ZIonosControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zIonos Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Ionos, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zIonos Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zIOX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zIOX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zIOX "));
    ui->labelzIOXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzIOXSyncStatus->setVisible(fShow);
}
